...
class NewsForm(forms.ModelForm):
    ...
    def clean_title(self):  #кастомный валидатор тайтла чтобы название не начиналось с цифры
...
## title = forms.CharField(max_length=255, label='Название', widget=forms.TextInput(
    # attrs={"class": "form-control", })) #создает виджеты формы
    # content = forms.CharField(label='Текст', required=False, widget=forms.Textarea
    # (attrs={
    #     "class": "form-control",
    #     "rows": 5
    # }))
    # is_published = forms.BooleanField(label='Опубликовано ', initial=True) #начальное значение галочки
